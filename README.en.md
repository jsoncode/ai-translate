...
src/engine/                     # translation engine: 26 single-purpose TS modules
...
The engine under `src/engine/` came from a self-developed page-translation engine (plain JS, hardened against many
real-world DOMs). It is now split into **26 focused TypeScript modules**, all passing `strict` type checking. The
split did not rewrite any function body — each module was verified byte-for-byte against the pre-split source — so
the scanning / composite-block / cache / re-apply / restore behaviour is unchanged.

Layers (dependencies point upwards):

| Module | Responsibility |
| --- | --- |
| `state.ts` | the single owner of mutable state (`runtime`) plus env constants and tag sets; imports no other engine module |
| `types.ts` | engine type contracts: config, status, items, selector slots, composite segments… |
| `logger.ts` / `lang.ts` / `markup.ts` / `media.ts` | logging, language/tag predicates, placeholders and media HTML |
| `composite*.ts` | composite blocks (inline elements / media mixed with text): request text, parsing, write-back, revert |
| `cache.ts` / `original.ts` / `selector.ts` / `write.ts` | route-bucketed cache, remembered originals (WeakMap), selector lookup, node writes |
| `viewport.ts` / `guard.ts` / `scan.ts` | viewport checks, write-boundary guards, node scanning |
| `batch.ts` / `parser.ts` / `stream.ts` / `transport.ts` | batching, the streaming `[index]` protocol, incremental apply, transport calls |
| `lifecycle.ts` / `mutation.ts` / `hooks.ts` / `listeners.ts` | revert, route changes, MutationObserver batching, scroll/title hooks |
| `api.ts` / `index.ts` | public API (run / stop / status / config) and the `window` export |

- Mutable state lives in one `runtime` object: ES modules cannot assign to imported bindings, and this makes every
  state change visible at the call site
- Before the split, `ai-translate-engine.js` was generated by `tools/build-engine.mjs`; the generator is now retired
  and **the engine source in this repo is the source of truth** (the history is kept above so upstream fixes can be
...
3. When changing translation behaviour, edit the relevant `src/engine/` module (`state.ts` is the only owner of
   mutable state — cross-module reads/writes go through `runtime.*`; type contracts live in `types.ts`)
...
> **Maintainer note before publishing**: the engine under `src/engine/` originates from an external, self-developed
> engine (it used to be generated by `tools/build-engine.mjs`, now maintained as TS modules inside this repo).
> Before releasing this repo under MIT, make sure you are allowed to license that upstream engine this way
> (e.g. get permission if it is a company asset, or document its separate license here and in `LICENSE`).
> The copyright holder in `LICENSE` can also be replaced with your name or organization.
