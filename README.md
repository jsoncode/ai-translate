...
<table>
  <tr>
    <td width="50%"><img src="./docs/popup-dark.png" alt="popup · 深色 · 翻译中"></td>
    <td width="50%"><img src="./docs/popup-light.png" alt="popup · 浅色 · 已完成"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./docs/options-dark.png" alt="设置页 · 深色"></td>
    <td width="50%"><img src="./docs/options-light.png" alt="设置页 · 浅色"></td>
  </tr>
</table>

设计取向：石墨中性底 + **唯一强调色**（信号青），层次只靠 1px 发丝线与字重，不用渐变、发光、毛玻璃；
状态、进度、错误一律**原位**给出，不弹气泡打断操作。主题支持**跟随系统 / 浅色 / 深色**三态。

<sub>截图来自真实构建产物（`dist/`）的渲染结果，数值为演示数据；主题在设置页左栏底部切换。</sub>
...
- **可视化配置**：popup 里一键开关 / 切翻译方向 / 切模型 / 看实时进度（`已翻译 17 / 48 段`），
  设置页里增删改模型并一键测试连通性；左栏导航 + 设置行的布局，支持跟随系统 / 浅色 / 深色主题
- **交互细节**：主操作随状态变化（翻译此页 / 关闭并还原），模型没配好时就在原地提示并给出口；
  `Ctrl/Cmd + Enter` 立即翻译、`Ctrl/Cmd + S` 保存设置；翻译中 0.7s、空闲 2.5s 的自适应轮询；有未保存改动会提示
- **无遥测**：不采集任何数据，没有自建上报
...
- `tests/`：配置收敛与校验、主题解析、模型客户端（SSE 解析、流式拼装、HTTP 错误、abort、非流式兜底）共 27 项
